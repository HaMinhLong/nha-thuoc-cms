...
import {
  ...
  filter,
} from '../../features/medicalFacilityGroup/medicalFacilityGroupSlice';
...
const MedicalFacilityGroup = ({ isMobile, intl, headerPage }) => {
  const [index, setIndex] = useState(-9999);
  let { id } = useParams();
  const userGroupId = localStorage.getItem('userGroupId');
  const dispatch = useDispatch();
  const list = useSelector(medicalFacilityGroup);
  ...
  const getList = () => {
    ...
    let params = {
      ...
      attributes: 'id,medicalFacilityGroupName,status,createdAt',
    };
    let values = {};
    ...
    dispatch({
      type: 'medicalFacilityGroup/fetch',
      payload: params,
      callback: (res) => {
        setLoading(false);
  ...
  const handleStatus = (value, row) => {
    ...
    dispatch({
      type: 'medicalFacilityGroup/updateStatus',
      payload: {
        id: row.id,
        params: item,
      },
  ...
  const saveRow = () => {
    const addItem = {
      ...dataEdit,
      medicalFacilityGroupName:
        (dataEdit.medicalFacilityGroupName &&
          dataEdit.medicalFacilityGroupName.trim()) ||
        '',
      medicalFacilityGroupNameOld:
        (dataEdit.medicalFacilityGroupName &&
          dataEdit.medicalFacilityGroupName.trim()) ||
        '',
    };
    if (
      !(
        addItem.medicalFacilityGroupName &&
        addItem.medicalFacilityGroupName.trim()
      ) ||
      (addItem.medicalFacilityGroupName &&
        addItem.medicalFacilityGroupName.trim() &&
        addItem.medicalFacilityGroupName.trim().length > 50)
    ) {
      openNotification(
        'error',
        intl.formatMessage({ id: 'app.medicalFacilityGroup.noti.col0' }),
        '#fff1f0'
      );
      return;
    }
    if (addItem.id > 0) {
      dispatch({
        type: 'medicalFacilityGroup/update',
        payload: {
          id: addItem.id,
          params: {
            ...addItem,
              ...
              intl.formatMessage(
                { id: 'app.common.create.success' },
                {
                  name: intl.formatMessage({
                    id: 'app.medicalFacilityGroup.list.title',
                  }),
                }
              ),
              '#f6ffed'
  ...
  const handleTableChange = (pagination, filters, sorter) => {
    ...
    const queryName = {
      medicalFacilityGroupName:
        queryFilter.medicalFacilityGroupName &&
        queryFilter.medicalFacilityGroupName.trim(),
    ...
    if (
      ...
      delete queryName.medicalFacilityGroupName;
    }
  ...
  const handleSearch = (values) => {
    ...
    const queryName = {
      medicalFacilityGroupName:
        values.medicalFacilityGroupName &&
        values.medicalFacilityGroupName.trim(),
  ...
  const renderForm = () => {
    ...
    return (
      <Form
        ...
        initialValues={{
          medicalFacilityGroupName: filter.medicalFacilityGroupName || '',
      ...
      >
        <Row gutter={{ md: 0, lg: 8, xl: 16 }}>
          <Col xs={24} md={12} xl={8}>
            <FormItem
              name="medicalFacilityGroupName"
              label={
                <FormattedMessage id="app.medicalFacilityGroup.list.col0" />
              }
              {...formItemLayout}
            >
              <Input
                placeholder={intl.formatMessage({
                  id: 'app.medicalFacilityGroup.search.col0',
                })}
                size="small"
              />
            </FormItem>
          ...
          <Col xl={8} md={12} xs={24}>
            <FormItem
              name="status"
              label={
                <FormattedMessage id="app.medicalFacilityGroup.list.col2" />
              }
              {...formItemLayout}
            >
              <Select
  ...
  const deleteRecord = (id) => {
    dispatch({
      type: 'medicalFacilityGroup/delete',
      payload: {
        id: id,
      },
      callback: (res) => {
  ...
  const columns = [
    ...
    {
      dataIndex: 'medicalFacilityGroupName',
      name: 'medicalFacilityGroupName',
      width: isMobile ? 150 : '15%',
      title: <FormattedMessage id="app.medicalFacilityGroup.list.col0" />,
      align: 'left',
      sorter: () => {},
      fixed: isMobile,
      render: (text, record) => {
        if (record.id === keyEdit) {
          return (
            <Input
              placeholder={intl.formatMessage({
                id: 'app.medicalFacilityGroup.list.name',
              })}
              value={dataEdit.medicalFacilityGroupName}
              onChange={(e) =>
                setDataEdit({
                  ...dataEdit,
                  medicalFacilityGroupName: e.target.value,
                })
              }
    ...
    },
    {
      dataIndex: 'status',
      name: 'status',
      title: <FormattedMessage id="app.medicalFacilityGroup.list.col2" />,
      align: 'center',
    ...
    <>
      {permissions ? (
        <>
          ...
          <HeaderContent
            title={
              <FormattedMessage id="app.medicalFacilityGroup.list.header" />
            }
            action={
              <React.Fragment>
                {permissions.isAdd && (
                  <Tooltip
                    title={
                      !isMobile &&
                      intl.formatMessage({
                        id: 'app.medicalFacilityGroup.create.header',
                      })
                    }
                  >
                    <Button
...
export default MedicalFacilityGroup;
