...
import { specialist, filter } from '../../features/specialist/specialistSlice';
...
const Specialist = ({ isMobile, intl, headerPage }) => {
  const [index, setIndex] = useState(-9999);
  let { id } = useParams();
  const userGroupId = localStorage.getItem('userGroupId');
  const dispatch = useDispatch();
  const list = useSelector(specialist);
  ...
  const getList = () => {
    ...
    let params = {
      ...
      attributes: 'id,specialistName,status,createdAt',
    };
    let values = {};
  ...
  const saveRow = () => {
    const addItem = {
      ...dataEdit,
      specialistName:
        (dataEdit.specialistName && dataEdit.specialistName.trim()) || '',
      specialistNameOld:
        (dataEdit.specialistName && dataEdit.specialistName.trim()) || '',
    };
    if (
      !(addItem.specialistName && addItem.specialistName.trim()) ||
      (addItem.specialistName &&
        addItem.specialistName.trim() &&
        addItem.specialistName.trim().length > 50)
    ) {
      openNotification(
        'error',
        intl.formatMessage({ id: 'app.specialist.noti.col0' }),
        '#fff1f0'
      );
      return;
    }
  ...
  const handleTableChange = (pagination, filters, sorter) => {
    ...
    const queryName = {
      specialistName:
        queryFilter.specialistName && queryFilter.specialistName.trim(),
      status: queryFilter && queryFilter.status,
      fromDate: fromDate,
      toDate: toDate,
    };
    if (!(queryFilter.specialistName && queryFilter.specialistName.trim())) {
      delete queryName.specialistName;
    }
  ...
  const handleSearch = (values) => {
    ...
    const toDate =
      rangeValue.length > 0
        ...
        : '';
    const queryName = {
      specialistName: values.specialistName && values.specialistName.trim(),
      status: values && values.status,
      fromDate: fromDate,
      toDate: toDate,
    };
    if (!(values.specialistName && values.specialistName.trim())) {
      delete queryName.specialistName;
    }
    if (!values.status) {
      delete queryName.status;
  ...
  const renderForm = () => {
    ...
    return (
      <Form
        ...
        initialValues={{
          specialistName: filter.specialistName || '',
      ...
      >
        <Row gutter={{ md: 0, lg: 8, xl: 16 }}>
          <Col xs={24} md={12} xl={8}>
            <FormItem
              name="specialistName"
              label={<FormattedMessage id="app.specialist.list.col0" />}
              {...formItemLayout}
            >
              <Input
                placeholder={intl.formatMessage({
                  id: 'app.specialist.search.col0',
                })}
                size="small"
              />
            </FormItem>
          ...
          <Col xl={8} md={12} xs={24}>
            <FormItem
              name="status"
              label={<FormattedMessage id="app.specialist.list.col2" />}
              {...formItemLayout}
            >
              <Select
                allowClear
  ...
  const columns = [
      ...
      render: (text, record) => {
        if (record.id === keyEdit) {
          return (
            <Input
              placeholder={intl.formatMessage({
                id: 'app.specialist.list.name',
              })}
              value={dataEdit.specialistName}
              onChange={(e) =>
                setDataEdit({ ...dataEdit, specialistName: e.target.value })
              }
    ...
    },
    {
      dataIndex: 'status',
      name: 'status',
      title: <FormattedMessage id="app.specialist.list.col2" />,
      align: 'center',
    ...
    <>
      {permissions ? (
        <>
          {headerPage}
          <HeaderContent
            title={<FormattedMessage id="app.specialist.list.header" />}
            action={
              <React.Fragment>
                {permissions.isAdd && (
                  <Tooltip
