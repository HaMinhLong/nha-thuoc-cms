...
import { unit, filter } from '../../features/unit/unitSlice';
...
const Unit = ({ isMobile, intl, headerPage }) => {
  ...
  const getList = () => {
    ...
    let params = {
      ...
      attributes: 'id,unitName,status,createdAt',
    };
    let values = {};
  ...
  const saveRow = () => {
    const addItem = {
      ...dataEdit,
      unitName: (dataEdit.unitName && dataEdit.unitName.trim()) || '',
      unitNameOld: (dataEdit.unitName && dataEdit.unitName.trim()) || '',
    };
    if (
      !(addItem.unitName && addItem.unitName.trim()) ||
      (addItem.unitName &&
        addItem.unitName.trim() &&
        addItem.unitName.trim().length > 50)
    ) {
      openNotification(
        'error',
        intl.formatMessage({ id: 'app.unit.noti.col0' }),
        '#fff1f0'
      );
      return;
    }
  ...
  const handleTableChange = (pagination, filters, sorter) => {
    ...
    const toDate =
      rangeValue.length > 0
        ...
        : '';
    const queryName = {
      unitName: queryFilter.unitName && queryFilter.unitName.trim(),
      status: queryFilter && queryFilter.status,
      fromDate: fromDate,
      toDate: toDate,
    };
    if (!(queryFilter.unitName && queryFilter.unitName.trim())) {
      delete queryName.unitName;
    }
  ...
  const handleSearch = (values) => {
    ...
    const toDate =
      rangeValue.length > 0
        ...
        : '';
    const queryName = {
      unitName: values.unitName && values.unitName.trim(),
      ...
      >
        <Row gutter={{ md: 0, lg: 8, xl: 16 }}>
          <Col xs={24} md={12} xl={8}>
            <FormItem
              name="unitName"
              label={<FormattedMessage id="app.unit.list.col0" />}
              {...formItemLayout}
            >
              <Input
                placeholder={intl.formatMessage({
                  id: 'app.unit.search.col0',
                })}
                size="small"
              />
            </FormItem>
          ...
          <Col xl={8} md={12} xs={24}>
            <FormItem
              name="status"
              label={<FormattedMessage id="app.unit.list.col2" />}
              {...formItemLayout}
            >
              <Select
                allowClear
  ...
  const columns = [
    ...
    },
    {
      ...
      title: <FormattedMessage id="app.unit.list.col0" />,
      align: 'left',
      sorter: () => {},
      fixed: isMobile,
      render: (text, record) => {
        if (record.id === keyEdit) {
          return (
            <Input
              placeholder={intl.formatMessage({
                id: 'app.unit.list.name',
              })}
              value={dataEdit.unitName}
              onChange={(e) =>
                setDataEdit({ ...dataEdit, unitName: e.target.value })
              }
    ...
    },
    {
      dataIndex: 'status',
      name: 'status',
      title: <FormattedMessage id="app.unit.list.col2" />,
      align: 'center',
    ...
    <>
      {permissions ? (
        <>
          {headerPage}
          <HeaderContent
            title={<FormattedMessage id="app.unit.list.header" />}
            action={
              <React.Fragment>
                {permissions.isAdd && (
                  <Tooltip
