...
import { medicine, filter } from '../../features/medicine/medicineSlice';
...
import MedicineModal from '../../components/ModalPage/MedicineModal';
import MedicineTypeSelect from '../../components/Common/MedicineTypeSelect';
...
const Customer = ({ isMobile, intl, headerPage }) => {
  ...
  const list = useSelector(medicine);
  ...
  const getList = () => {
    ...
    let params = {
      ...
      attributes:
        'id,medicineName,registrationNumber,standard,activeIngredientName,concentration,status,createdAt',
    };
    let values = {};
  ...
  const handleTableChange = (pagination, filters, sorter) => {
    ...
    const toDate =
      rangeValue.length > 0
        ...
        : '';
    const queryName = {
      medicineName: queryFilter.medicineName && queryFilter.medicineName.trim(),
      medicineTypeId: queryFilter && queryFilter.medicineTypeId,
      producerId: queryFilter && queryFilter.producerId,
      status: queryFilter && queryFilter.status,
      fromDate: fromDate,
      toDate: toDate,
      healthFacilityId,
    };
    if (!(queryFilter.medicineName && queryFilter.medicineName.trim())) {
      delete queryName.medicineName;
    }
    if (!queryFilter.medicineTypeId) {
      delete queryName.medicineTypeId;
    }
  ...
  const handleSearch = (values) => {
    ...
    const queryName = {
      ...
      medicineTypeId: values && values.medicineTypeId,
      producerId: values && values.producerId,
  ...
  const handleExpandedRowRender = (record) => (
    ...
    >
      ...
      <Descriptions.Item
        label={
          <strong>
            {intl.formatMessage({ id: 'app.medicine.list.col8' })}
          </strong>
        }
      >
  ...
  const renderForm = () => {
    ...
    return (
      <Form
        ...
        initialValues={{
          medicineName: filter.medicineName || '',
      ...
      >
        <Row gutter={{ md: 0, lg: 8, xl: 16 }}>
          <Col xs={24} md={12} xl={8}>
            <FormItem
              name="medicineName"
              label={<FormattedMessage id="app.medicine.list.col0" />}
              {...formItemLayout}
            >
              <Input
                placeholder={intl.formatMessage({
                  id: 'app.medicine.search.col0',
                })}
                size="small"
              />
            </FormItem>
          ...
          <Col xs={24} md={12} xl={8}>
            <FormItem
              name="medicineTypeId"
              label={<FormattedMessage id="app.medicine.list.col7" />}
              {...formItemLayout}
            >
              <MedicineTypeSelect
                placeholder={intl.formatMessage({
                  id: 'app.medicine.search.col1',
                })}
                allowClear
                size="small"
              />
          ...
          <Col xs={24} md={12} xl={8}>
            <FormItem
              name="producerId"
              label={<FormattedMessage id="app.medicine.list.col10" />}
              {...formItemLayout}
            >
              <ProducerSelect
                placeholder={intl.formatMessage({
                  id: 'app.medicine.search.col2',
                })}
                allowClear
                size="small"
              />
          ...
          <Col xl={8} md={12} xs={24}>
            <FormItem
              name="status"
              label={<FormattedMessage id="app.medicine.list.col11" />}
              {...formItemLayout}
            >
              <Select
                allowClear
  ...
  const columns = [
    ...
    },
    {
      ...
      title: <FormattedMessage id="app.medicine.list.col0" />,
      align: 'left',
      sorter: () => {},
      fixed: isMobile,
    },
    {
      ...
      width: isMobile ? 150 : '15%',
      title: <FormattedMessage id="app.medicine.list.col1" />,
      align: 'center',
      sorter: () => {},
    },
    {
      ...
      name: 'standard',
      width: isMobile ? 150 : '15%',
      title: <FormattedMessage id="app.medicine.list.col2" />,
      align: 'center',
      sorter: () => {},
    },
    {
      ...
      name: 'medicineType',
      width: isMobile ? 150 : '15%',
      title: <FormattedMessage id="app.medicine.list.col7" />,
      align: 'center',
      sorter: () => {},
      render: (cell) => <span>{cell.medicineTypeName}</span>,
    },
    {
      dataIndex: 'producer',
      name: 'producer',
      width: isMobile ? 150 : '15%',
      title: <FormattedMessage id="app.medicine.list.col10" />,
      align: 'center',
      sorter: () => {},
      render: (cell) => <span>{cell.producerName}</span>,
    },
    {
      dataIndex: 'createdAt',
    ...
    },
    {
      dataIndex: 'status',
      name: 'status',
      title: <FormattedMessage id="app.medicine.list.col11" />,
      align: 'center',
    ...
    <>
      {permissions ? (
        <>
          {headerPage}
          <HeaderContent
            title={<FormattedMessage id="app.medicine.list.header" />}
            action={
              <React.Fragment>
                {permissions.isAdd && (
                  <Tooltip
      ...
      <MedicineModal
        ...
        titleModal={intl.formatMessage({ id: 'app.medicine.list.title' })}
        dataEdit={dataEdit}
        getList={getList}
      />
    </>
