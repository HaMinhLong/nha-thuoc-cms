...
import {
  medicineType,
  filter,
} from '../../features/medicineType/medicineTypeSlice';
...
const MedicineType = ({ isMobile, intl, headerPage }) => {
  const [index, setIndex] = useState(-9999);
  let { id } = useParams();
  const userGroupId = localStorage.getItem('userGroupId');
  const dispatch = useDispatch();
  const list = useSelector(medicineType);
  ...
  const getList = () => {
    ...
    let params = {
      ...
      attributes: 'id,medicineTypeName,status,createdAt',
    };
    let values = {};
  ...
  const handleStatus = (value, row) => {
    ...
    dispatch({
      type: 'medicineType/updateStatus',
      payload: {
        id: row.id,
        params: item,
      },
  ...
  const saveRow = () => {
    const addItem = {
      ...dataEdit,
      medicineTypeName:
        (dataEdit.medicineTypeName && dataEdit.medicineTypeName.trim()) || '',
      medicineTypeNameOld:
        (dataEdit.medicineTypeName && dataEdit.medicineTypeName.trim()) || '',
    };
    if (
      !(addItem.medicineTypeName && addItem.medicineTypeName.trim()) ||
      (addItem.medicineTypeName &&
        addItem.medicineTypeName.trim() &&
        addItem.medicineTypeName.trim().length > 50)
    ) {
      openNotification(
        'error',
        intl.formatMessage({ id: 'app.medicineType.noti.col0' }),
        '#fff1f0'
      );
      return;
    }
              ...
              intl.formatMessage(
                { id: 'app.common.create.success' },
                {
                  name: intl.formatMessage({
                    id: 'app.medicineType.list.title',
                  }),
                }
              ),
              '#f6ffed'
  ...
  const handleTableChange = (pagination, filters, sorter) => {
    ...
    const queryName = {
      medicineTypeName:
        queryFilter.medicineTypeName && queryFilter.medicineTypeName.trim(),
    ...
    if (
      ...
      delete queryName.medicineTypeName;
    }
  ...
  const handleSearch = (values) => {
    ...
    const queryName = {
      medicineTypeName:
        values.medicineTypeName && values.medicineTypeName.trim(),
      status: values && values.status,
      fromDate: fromDate,
      toDate: toDate,
    };
    if (!(values.medicineTypeName && values.medicineTypeName.trim())) {
      delete queryName.medicineTypeName;
    }
    if (!values.status) {
      delete queryName.status;
  ...
  const renderForm = () => {
    ...
    return (
      <Form
        ...
        initialValues={{
          medicineTypeName: filter.medicineTypeName || '',
      ...
      >
        <Row gutter={{ md: 0, lg: 8, xl: 16 }}>
          <Col xs={24} md={12} xl={8}>
            <FormItem
              name="medicineTypeName"
              label={<FormattedMessage id="app.medicineType.list.col0" />}
              {...formItemLayout}
            >
              <Input
                placeholder={intl.formatMessage({
                  id: 'app.medicineType.search.col0',
                })}
                size="small"
              />
            </FormItem>
          ...
          <Col xl={8} md={12} xs={24}>
            <FormItem
              name="status"
              label={<FormattedMessage id="app.medicineType.list.col2" />}
              {...formItemLayout}
            >
              <Select
                allowClear
  ...
  const columns = [
    ...
    {
      dataIndex: 'medicineTypeName',
      name: 'medicineTypeName',
      width: isMobile ? 150 : '15%',
      title: <FormattedMessage id="app.medicineType.list.col0" />,
      align: 'left',
      sorter: () => {},
      fixed: isMobile,
      render: (text, record) => {
        if (record.id === keyEdit) {
          return (
            <Input
              placeholder={intl.formatMessage({
                id: 'app.medicineType.list.name',
              })}
              value={dataEdit.medicineTypeName}
              onChange={(e) =>
                setDataEdit({ ...dataEdit, medicineTypeName: e.target.value })
              }
    ...
    },
    {
      dataIndex: 'status',
      name: 'status',
      title: <FormattedMessage id="app.medicineType.list.col2" />,
      align: 'center',
    ...
    <>
      {permissions ? (
        <>
          {headerPage}
          <HeaderContent
            title={<FormattedMessage id="app.medicineType.list.header" />}
            action={
              <React.Fragment>
                {permissions.isAdd && (
                  <Tooltip
                    title={
                      !isMobile &&
                      intl.formatMessage({
                        id: 'app.medicineType.create.header',
                      })
                    }
                  >
                    <Button
