...
import {
  clinicPrescription,
  filter,
} from '../../features/clinicPrescription/clinicPrescriptionSlice';
import '../../utils/css/styleList.scss';
import moment from 'moment';
import filterIcon from '../../static/web/images/filter.svg';
import { formatNumber } from '../../utils/utils';
import ClinicPrescriptionModal from '../../components/ModalPage/ClinicPrescriptionModal';
...
const ClinicPrescription = ({ isMobile, intl, headerPage }) => {
  let { id } = useParams();
  const userGroupId = localStorage.getItem('userGroupId');
  const healthFacilityId = localStorage.getItem('healthFacilityId');
  const dispatch = useDispatch();
  const list = useSelector(clinicPrescription);
    ...
    <React.Fragment>
      ...
      {permissions ? (
        <>
          {headerPage}
          <HeaderContent
            title={<FormattedMessage id="app.clinicPrescription.list.header" />}
          >
      ...
      <ClinicPrescriptionModal
        ...
        dataCustomer={dataEdit}
        dataEdit={{ id: dataEdit?.id }}
      />
    </React.Fragment>
  );
};
