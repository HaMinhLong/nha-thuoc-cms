...
import { packages, filter } from '../../features/packages/packagesSlice';
...
const Unit = ({ isMobile, intl, headerPage }) => {
  const [index, setIndex] = useState(-9999);
  let { id } = useParams();
  const userGroupId = localStorage.getItem('userGroupId');
  const dispatch = useDispatch();
  const list = useSelector(packages);
  ...
  const getList = () => {
    ...
    let params = {
      ...
      attributes: 'id,packageName,status,createdAt',
    };
    let values = {};
  ...
  const saveRow = () => {
    const addItem = {
      ...dataEdit,
      packageName: (dataEdit.packageName && dataEdit.packageName.trim()) || '',
      packageNameOld:
        (dataEdit.packageName && dataEdit.packageName.trim()) || '',
    };
    if (
      !(addItem.packageName && addItem.packageName.trim()) ||
      (addItem.packageName &&
        addItem.packageName.trim() &&
        addItem.packageName.trim().length > 50)
    ) {
      openNotification(
        'error',
        intl.formatMessage({ id: 'app.packages.noti.col0' }),
        '#fff1f0'
      );
      return;
    }
  ...
  const handleTableChange = (pagination, filters, sorter) => {
    ...
    const toDate =
      rangeValue.length > 0
        ...
        : '';
    const queryName = {
      packageName: queryFilter.packageName && queryFilter.packageName.trim(),
      status: queryFilter && queryFilter.status,
      fromDate: fromDate,
      toDate: toDate,
    };
    if (!(queryFilter.packageName && queryFilter.packageName.trim())) {
      delete queryName.packageName;
    }
  ...
  const handleSearch = (values) => {
    ...
    const toDate =
      rangeValue.length > 0
        ...
        : '';
    const queryName = {
      packageName: values.packageName && values.packageName.trim(),
      ...
      >
        <Row gutter={{ md: 0, lg: 8, xl: 16 }}>
          <Col xs={24} md={12} xl={8}>
            <FormItem
              name="packageName"
              label={<FormattedMessage id="app.packages.list.col0" />}
              {...formItemLayout}
            >
              <Input
                placeholder={intl.formatMessage({
                  id: 'app.packages.search.col0',
                })}
                size="small"
              />
            </FormItem>
          ...
          <Col xl={8} md={12} xs={24}>
            <FormItem
              name="status"
              label={<FormattedMessage id="app.packages.list.col2" />}
              {...formItemLayout}
            >
              <Select
                allowClear
  ...
  const columns = [
    ...
    },
    {
      ...
      title: <FormattedMessage id="app.packages.list.col0" />,
      align: 'left',
      sorter: () => {},
      fixed: isMobile,
      render: (text, record) => {
        if (record.id === keyEdit) {
          return (
            <Input
              placeholder={intl.formatMessage({
                id: 'app.packages.list.name',
              })}
              value={dataEdit.packageName}
              onChange={(e) =>
                setDataEdit({ ...dataEdit, packageName: e.target.value })
              }
    ...
    },
    {
      dataIndex: 'status',
      name: 'status',
      title: <FormattedMessage id="app.packages.list.col2" />,
      align: 'center',
    ...
    <>
      {permissions ? (
        <>
          {headerPage}
          <HeaderContent
            title={<FormattedMessage id="app.packages.list.header" />}
            action={
              <React.Fragment>
                {permissions.isAdd && (
                  <Tooltip
