...
import {
  clinicService,
  filter,
} from '../../features/clinicService/clinicServiceSlice';
...
import ClinicServiceDrawer from '../../components/DrawerPage/ClinicServiceDrawer';
import ClinicServicePackageSelect from '../../components/Common/ClinicServicePackageSelect';
import { Redirect } from 'react-router-dom';

const FormItem = Form.Item;
const { RangePicker } = DatePicker;
const PAGE_SIZE = process.env.REACT_APP_PAGE_SIZE;
const ClinicService = ({ isMobile, intl, headerPage }) => {
  let { id } = useParams();
  const userGroupId = localStorage.getItem('userGroupId');
  const healthFacilityId = localStorage.getItem('healthFacilityId');
  const dispatch = useDispatch();
  const list = useSelector(clinicService);
  ...
  const getList = () => {
    ...
    let params = {
      ...
      attributes: 'id,clinicServiceName,status,createdAt',
    };
    let values = {};
  ...
  const handleStatus = (value, row) => {
    ...
    dispatch({
      type: 'clinicService/updateStatus',
      payload: {
        id: row.id,
        params: item,
      },
  ...
  const handleTableChange = (pagination, filters, sorter) => {
    ...
    const queryName = {
      clinicServiceName:
        queryFilter.clinicServiceName && queryFilter.clinicServiceName.trim(),
      clinicServicePackageId: queryFilter && queryFilter.clinicServicePackageId,
    ...
    if (
      ...
      delete queryName.clinicServiceName;
    }
    if (!queryFilter.clinicServicePackageId) {
      delete queryName.clinicServicePackageId;
    }
  ...
  const handleSearch = (values) => {
    ...
    const queryName = {
      ...
      clinicServicePackageId: values && values.clinicServicePackageId,
      status: values && values.status,
      fromDate: fromDate,
      toDate: toDate,
      healthFacilityId,
    };
    if (!(values.clinicServiceName && values.clinicServiceName.trim())) {
      delete queryName.clinicServiceName;
    }
    if (!values.clinicServicePackageId) {
      delete queryName.clinicServicePackageId;
    }
    if (!values.status) {
      delete queryName.status;
  ...
  const renderForm = () => {
    ...
    return (
      <Form
        ...
        initialValues={{
          clinicServiceName: filter.clinicServiceName || '',
          clinicServicePackageId: filter.clinicServicePackageId || '',
        ...
        <Row gutter={{ md: 0, lg: 8, xl: 16 }}>
          <Col xs={24} md={12} xl={8}>
            <FormItem
              ...
              label={<FormattedMessage id="app.clinicService.list.col0" />}
              {...formItemLayout}
            >
              <Input
                placeholder={intl.formatMessage({
                  id: 'app.clinicService.search.col0',
                })}
                size="small"
              />
            </FormItem>
          ...
          <Col xs={24} md={12} xl={8}>
            <FormItem
              ...
              label={<FormattedMessage id="app.clinicService.list.col1" />}
              {...formItemLayout}
            >
              <ClinicServicePackageSelect
                placeholder={intl.formatMessage({
                  id: 'app.clinicService.search.col1',
                })}
                allowClear
                size="small"
              />
          ...
          <Col xl={8} md={12} xs={24}>
            <FormItem
              name="status"
              label={<FormattedMessage id="app.clinicService.list.col5" />}
              {...formItemLayout}
            >
              <Select
                allowClear
  ...
  const columns = [
    ...
    },
    {
      ...
      title: <FormattedMessage id="app.clinicService.list.col0" />,
      align: 'left',
      sorter: () => {},
      fixed: isMobile,
    },
    {
      dataIndex: 'clinicServicePackage',
      name: 'clinicServicePackage',
      width: isMobile ? 150 : '15%',
      title: <FormattedMessage id="app.clinicService.list.col1" />,
      align: 'center',
      sorter: () => {},
      render: (cell) => <span>{cell.clinicServicePackageName}</span>,
    },
    {
      dataIndex: 'createdAt',
    ...
    },
    {
      dataIndex: 'status',
      name: 'status',
      title: <FormattedMessage id="app.clinicService.list.col5" />,
      align: 'center',
    ...
    <>
      {permissions ? (
        <>
          {headerPage}
          <HeaderContent
            title={<FormattedMessage id="app.clinicService.list.header" />}
            action={
              <React.Fragment>
                {permissions.isAdd && (
                  <Tooltip
      ...
      <ClinicServiceDrawer
        ...
        titleDrawer={intl.formatMessage({
          id: 'app.clinicService.list.title',
        })}
        dataEdit={dataEdit}
        getList={getList}
      />
