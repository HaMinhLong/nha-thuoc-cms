...
import { receipt, filter } from '../../features/receipt/receiptSlice';
import { FormattedMessage } from 'react-intl';
import { formatNumber, getTimeDistance } from '../../utils/utils';
import { useParams } from 'react-router-dom';
import { Redirect } from 'react-router-dom';
import Receipt from '../../components/BillTable/Receipt';
...
const ReceiptPage = ({ isMobile, intl, headerPage }) => {
  let { id } = useParams();
  const formRef = React.createRef();
  const userGroupId = localStorage.getItem('userGroupId');
  const healthFacilityId = localStorage.getItem('healthFacilityId');
  const list = useSelector(receipt);
      ...
      >
        <Row gutter={10} style={{ background: '#fff', margin: 0 }}>
          <Col xl={7} md={24} xs={24}>
            ...
            >
              {intl.formatMessage({ id: 'app.receipt.list' })}
            </p>
          </Col>
          <Col
            xl={10}
  ...
  const columns = [
    ...
    },
    {
      ...
      title: <FormattedMessage id="app.receipt.list.col0" />,
      align: 'left',
      sorter: () => {},
      fixed: isMobile,
    },
    ...
    {
      dataIndex: 'supplier',
      title: intl.formatMessage({ id: 'app.receipt.list.col5' }),
      align: 'center',
      width: !isMobile && '9%',
      sorter: () => {},
      render: (cell) => <span>{cell?.supplierName}</span>,
    },
    {
      dataIndex: 'status',
      title: intl.formatMessage({ id: 'app.receipt.list.col8' }),
      align: 'center',
      name: 'status',
      width: '2%',
      render: (cell, row) => (
