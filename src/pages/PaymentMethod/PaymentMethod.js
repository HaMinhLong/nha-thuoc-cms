...
const PaymentMethod = ({ isMobile, intl, headerPage }) => {
  const [index, setIndex] = useState(-9999);
  let { id } = useParams();
  const userGroupId = localStorage.getItem('userGroupId');
  const dispatch = useDispatch();
  const list = useSelector(paymentMethod);
  ...
  const getList = () => {
    ...
    let params = {
      ...
      attributes: 'id,paymentMethodName,status,createdAt',
    };
    let values = {};
  ...
  const handleStatus = (value, row) => {
    ...
    dispatch({
      type: 'paymentMethod/updateStatus',
      payload: {
        id: row.id,
        params: item,
      },
  ...
  const saveRow = () => {
    const addItem = {
      ...dataEdit,
      paymentMethodName:
        (dataEdit.paymentMethodName && dataEdit.paymentMethodName.trim()) || '',
      paymentMethodNameOld:
        (dataEdit.paymentMethodName && dataEdit.paymentMethodName.trim()) || '',
    };
    if (
      !(addItem.paymentMethodName && addItem.paymentMethodName.trim()) ||
      (addItem.paymentMethodName &&
        addItem.paymentMethodName.trim() &&
        addItem.paymentMethodName.trim().length > 50)
    ) {
      openNotification(
        'error',
        intl.formatMessage({ id: 'app.paymentMethod.noti.col0' }),
        '#fff1f0'
      );
      return;
    }
  ...
  const handleTableChange = (pagination, filters, sorter) => {
    ...
    const queryName = {
      paymentMethodName:
        queryFilter.paymentMethodName && queryFilter.paymentMethodName.trim(),
    ...
    if (
      ...
      delete queryName.paymentMethodName;
    }
  ...
  const handleSearch = (values) => {
    ...
    const queryName = {
      paymentMethodName:
        values.paymentMethodName && values.paymentMethodName.trim(),
      status: values && values.status,
      fromDate: fromDate,
      toDate: toDate,
    };
    if (!(values.paymentMethodName && values.paymentMethodName.trim())) {
      delete queryName.paymentMethodName;
    }
    if (!values.status) {
      delete queryName.status;
        ...
        <Row gutter={{ md: 0, lg: 8, xl: 16 }}>
          <Col xs={24} md={12} xl={8}>
            <FormItem
              ...
              label={<FormattedMessage id="app.paymentMethod.list.col0" />}
              {...formItemLayout}
            >
              <Input
                placeholder={intl.formatMessage({
                  id: 'app.paymentMethod.search.col0',
                })}
                size="small"
              />
            </FormItem>
          ...
          <Col xl={8} md={12} xs={24}>
            <FormItem
              name="status"
              label={<FormattedMessage id="app.paymentMethod.list.col2" />}
              {...formItemLayout}
            >
              <Select
                allowClear
  ...
  const columns = [
    ...
    },
    {
      ...
      title: <FormattedMessage id="app.paymentMethod.list.col0" />,
      align: 'left',
      sorter: () => {},
      fixed: isMobile,
      render: (text, record) => {
        if (record.id === keyEdit) {
          return (
            <Input
              ...
              value={dataEdit.paymentMethodName}
              onChange={(e) =>
                setDataEdit({ ...dataEdit, paymentMethodName: e.target.value })
              }
    ...
    },
    {
      dataIndex: 'status',
      name: 'status',
      title: <FormattedMessage id="app.paymentMethod.list.col2" />,
      align: 'center',
    ...
    <>
      {permissions ? (
        <>
          {headerPage}
          <HeaderContent
            title={<FormattedMessage id="app.paymentMethod.list.header" />}
            action={
              <React.Fragment>
                {permissions.isAdd && (
                  <Tooltip
                    title={
                      !isMobile &&
                      intl.formatMessage({
                        id: 'app.paymentMethod.create.header',
                      })
                    }
                  >
                    <Button
