...
const UserGroupRole = ({ isMobile, intl, headerPage }) => {
  ...
  const getList = () => {
    ...
    let params = {
      ...
      attributes:
        'id,menuName,menuParentId,userGroupId,menuId,isView,isAdd,isUpdate,isDelete,isBlock,isApprove,isReceipts,isPrescription,isResult',
    };
    let values = {};
  ...
  const columns = [
    ...
    {
      dataIndex: 'isReceipts',
      title: (
        <div style={{ marginLeft: '28px' }}>
          <CheckBox onChange={(e) => addAllRoles(e, 'isReceipts')} />
          &nbsp;
          {intl.formatMessage({ id: 'app.userGroupRole.list.isReceipts' })}
        </div>
      ),
      align: 'center',
      key: 'isReceipts',
    ...
    },
    {
      ...
      title: (
        <div style={{ marginLeft: '28px' }}>
          <CheckBox onChange={(e) => addAllRoles(e, 'isPrescription')} />
          &nbsp;
          {intl.formatMessage({ id: 'app.userGroupRole.list.isPrescription' })}
        </div>
      ),
      align: 'center',
      key: 'isPrescription',
    ...
    },
    {
      ...
      title: (
        <div style={{ marginLeft: '28px' }}>
          <CheckBox onChange={(e) => addAllRoles(e, 'isResult')} />
          &nbsp;{intl.formatMessage({ id: 'app.userGroupRole.list.isResult' })}
        </div>
      ),
      align: 'center',
      key: 'isResult',
      ...
      >
        <Table
          ...
          expandedRowKeys={difference.map(
            (item) =>
              `${item.id}-${item.isView}-${item.isAdd}-${item.isUpdate}-${item.isDelete}-${item.isBlock}-${item.isApprove}-${item.isReceipts}-${item.isPrescription}-${item.isResult}`
          )}
          bordered
          rowKey={(record) =>
            `${record.id}-${record.isView}-${record.isAdd}-${record.isUpdate}-${record.isDelete}-${record.isBlock}-${record.isApprove}-${record.isReceipts}-${record.isPrescription}-${record.isResult}`
          }
