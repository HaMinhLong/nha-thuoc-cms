...
import {
  ...
  filter,
} from '../../features/clinicServicePackage/clinicServicePackageSlice';
...
import ClinicServicePackageModal from '../../components/ModalPage/ClinicServicePackageModal';
import ClinicTypeSelect from '../../components/Common/ClinicTypeSelect';
...
const Customer = ({ isMobile, intl, headerPage }) => {
  ...
  const list = useSelector(clinicServicePackage);
  ...
  const getList = () => {
    ...
    let params = {
      ...
      attributes: 'id,clinicServicePackageName,status,createdAt',
    };
    let values = {};
    ...
    dispatch({
      type: 'clinicServicePackage/fetch',
      payload: params,
      callback: (res) => {
        setLoading(false);
  ...
  const handleStatus = (value, row) => {
    ...
    dispatch({
      type: 'clinicServicePackage/updateStatus',
      payload: {
        id: row.id,
        params: item,
      },
  ...
  const handleTableChange = (pagination, filters, sorter) => {
    ...
    const queryName = {
      clinicServicePackageName:
        queryFilter.clinicServicePackageName &&
        queryFilter.clinicServicePackageName.trim(),
      clinicTypeId: queryFilter && queryFilter.clinicTypeId,
    ...
    if (
      ...
      delete queryName.clinicServicePackageName;
    }
    if (!queryFilter.clinicTypeId) {
      delete queryName.clinicTypeId;
    }
  ...
  const handleSearch = (values) => {
    ...
    const queryName = {
      ...
      clinicTypeId: values && values.clinicTypeId,
  ...
  const renderForm = () => {
    ...
    return (
      <Form
        ...
        initialValues={{
          clinicServicePackageName: filter.clinicServicePackageName || '',
          clinicTypeId: filter.clinicTypeId || '',
        ...
        <Row gutter={{ md: 0, lg: 8, xl: 16 }}>
          <Col xs={24} md={12} xl={8}>
            <FormItem
              ...
              label={
                <FormattedMessage id="app.clinicServicePackage.list.col0" />
              }
              {...formItemLayout}
            >
              <Input
                placeholder={intl.formatMessage({
                  id: 'app.clinicServicePackage.search.col0',
                })}
                size="small"
              />
            </FormItem>
          ...
          <Col xs={24} md={12} xl={8}>
            ...
            >
              <ClinicTypeSelect
                placeholder={intl.formatMessage({
                  id: 'app.clinicServicePackage.search.col1',
                })}
                allowClear
                size="small"
              />
          ...
          <Col xl={8} md={12} xs={24}>
            <FormItem
              name="status"
              label={
                <FormattedMessage id="app.clinicServicePackage.list.col5" />
              }
              {...formItemLayout}
            >
              <Select
  ...
  const deleteRecord = (id) => {
    dispatch({
      type: 'clinicServicePackage/delete',
      payload: {
        id: id,
      },
      callback: (res) => {
  ...
  const columns = [
    ...
    {
      dataIndex: 'clinicServicePackageName',
      name: 'clinicServicePackageName',
      width: isMobile ? 150 : '15%',
      title: <FormattedMessage id="app.clinicServicePackage.list.col0" />,
      align: 'left',
      sorter: () => {},
      fixed: isMobile,
    },
    {
      ...
      name: 'clinicType',
      width: isMobile ? 150 : '15%',
      title: <FormattedMessage id="app.clinicServicePackage.list.col1" />,
      align: 'center',
      sorter: () => {},
      render: (cell) => <span>{cell.clinicTypeName}</span>,
    },
    {
      dataIndex: 'createdAt',
    ...
    },
    {
      dataIndex: 'status',
      name: 'status',
      title: <FormattedMessage id="app.clinicServicePackage.list.col5" />,
      align: 'center',
    ...
    <>
      {permissions ? (
        <>
          ...
          <HeaderContent
            title={
              <FormattedMessage id="app.clinicServicePackage.list.header" />
            }
            action={
              <React.Fragment>
                {permissions.isAdd && (
      ...
      <ClinicServicePackageModal
        intl={intl}
        isMobile={isMobile}
        visible={visibleModal}
        titleModal={intl.formatMessage({
          id: 'app.clinicServicePackage.list.title',
        })}
        dataEdit={dataEdit}
        getList={getList}
      />
