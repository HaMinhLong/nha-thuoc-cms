...
import { province, filter } from '../../features/province/provinceSlice';
...
import ProvinceDrawer from '../../components/DrawerPage/ProvinceDrawer';
import { Redirect } from 'react-router-dom';

const FormItem = Form.Item;
const { RangePicker } = DatePicker;
const PAGE_SIZE = process.env.REACT_APP_PAGE_SIZE;
const Province = ({ isMobile, intl, headerPage }) => {
  let { id } = useParams();
  const userGroupId = localStorage.getItem('userGroupId');
  const dispatch = useDispatch();
  const list = useSelector(province);
  ...
  const getList = () => {
    ...
    let params = {
      ...
      attributes: 'id,provinceName,status,createdAt',
    };
    let values = {};
  ...
  const handleTableChange = (pagination, filters, sorter) => {
    ...
    const toDate =
      rangeValue.length > 0
        ...
        : '';
    const queryName = {
      provinceName: queryFilter.provinceName && queryFilter.provinceName.trim(),
      status: queryFilter && queryFilter.status,
      fromDate: fromDate,
      toDate: toDate,
    };
    if (!(queryFilter.provinceName && queryFilter.provinceName.trim())) {
      delete queryName.provinceName;
    }
  ...
  const handleSearch = (values) => {
    ...
    if (!(values.provinceName && values.provinceName.trim())) {
      delete queryName.provinceName;
    }
    if (!values.status) {
      delete queryName.status;
      ...
      >
        <Row gutter={{ md: 0, lg: 8, xl: 16 }}>
          <Col xs={24} md={12} xl={8}>
            <FormItem
              name="provinceName"
              label={<FormattedMessage id="app.province.list.col0" />}
              {...formItemLayout}
            >
              <Input
                placeholder={intl.formatMessage({
                  id: 'app.province.search.col0',
                })}
                size="small"
              />
            </FormItem>
          ...
          <Col xl={8} md={12} xs={24}>
            <FormItem
              name="status"
              label={<FormattedMessage id="app.province.list.col2" />}
              {...formItemLayout}
            >
              <Select
                allowClear
  ...
  const columns = [
    ...
    },
    {
      ...
      title: <FormattedMessage id="app.province.list.col0" />,
      align: 'left',
      sorter: () => {},
      fixed: isMobile,
    },
    ...
    },
    {
      dataIndex: 'status',
      name: 'status',
      title: <FormattedMessage id="app.province.list.col2" />,
      align: 'center',
    ...
    <>
      {permissions ? (
        <>
          {headerPage}
          <HeaderContent
            title={<FormattedMessage id="app.province.list.header" />}
            action={
              <React.Fragment>
                {permissions.isAdd && (
                  <Tooltip
                    title={
                      !isMobile &&
                      intl.formatMessage({ id: 'app.province.create.header' })
                    }
                  >
                    <Button
                      icon={
      ...
      <ProvinceDrawer
        ...
        titleDrawer={intl.formatMessage({ id: 'app.province.list.title' })}
        dataEdit={dataEdit}
        getList={getList}
      />
    </>
