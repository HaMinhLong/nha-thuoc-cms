...
const UserGroupSelect = ({
  ...
  const fetch = (
    ...
    if (!searchValue || (searchValue && !searchValue.trim())) {
      delete tfilter.specialistName;
    }
    const params = {
      filter: JSON.stringify(tfilter),
      range: JSON.stringify([pagesize * (current - 1), current * pagesize]),
      sort: JSON.stringify(['specialistName', 'ASC']),
      attributes: 'id,specialistName',
    };
    dispatch({
      type: 'specialist/fetchLazyLoading',
      payload: params,
      select: current !== 1,
      callback: (result) => {
  ...
  const dataRender = renderData(data ? dataNew : dataArr);
  return (
    <React.Fragment>
      <Select
        mode={mode}
