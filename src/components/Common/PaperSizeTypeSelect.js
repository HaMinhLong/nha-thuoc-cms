...
const PaperSizeTypeSelect = ({
  ...
  const fetch = (
    ...
    if (!searchValue || (searchValue && !searchValue.trim())) {
      delete tfilter.paperSizeTypeName;
    }
    const params = {
      filter: JSON.stringify(tfilter),
      range: JSON.stringify([pagesize * (current - 1), current * pagesize]),
      sort: JSON.stringify(['paperSizeTypeName', 'ASC']),
      attributes: 'id,paperSizeTypeName',
    };
    dispatch({
      type: 'paperSizeType/fetchLazyLoading',
      payload: params,
      select: current !== 1,
      callback: (result) => {
...
export default PaperSizeTypeSelect;
