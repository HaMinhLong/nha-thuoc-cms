...
const UserGroupSelect = ({
  ...
  const fetch = (
    ...
    const pagesize = 20;
    const tfilter = {
      medicalFacilityGroupName: searchValue,
      status: 1,
    };
    if (getAll) {
    ...
    if (!searchValue || (searchValue && !searchValue.trim())) {
      delete tfilter.medicalFacilityGroupName;
    }
    const params = {
      filter: JSON.stringify(tfilter),
      range: JSON.stringify([pagesize * (current - 1), current * pagesize]),
      sort: JSON.stringify(['medicalFacilityGroupName', 'ASC']),
      attributes: 'id,medicalFacilityGroupName',
    };
    dispatch({
      type: 'medicalFacilityGroup/fetchLazyLoading',
      payload: params,
      select: current !== 1,
      callback: (result) => {
        if (result && result.success === true) {
          const data =
            ...
            result.results.list.map((data) => ({
              valueState: data?.id,
              text: data.medicalFacilityGroupName,
            }));
          setTotalItems(
            result &&
