...
const PrintFormSelect = ({
  ...
  const fetch = (
    ...
    if (!searchValue || (searchValue && !searchValue.trim())) {
      delete tfilter.printFormName;
    }
    const params = {
      filter: JSON.stringify(tfilter),
      range: JSON.stringify([pagesize * (current - 1), current * pagesize]),
      sort: JSON.stringify(['printFormName', 'ASC']),
      attributes: 'id,printFormName',
    };
    dispatch({
      type: 'printForm/fetchLazyLoading',
      payload: params,
      select: current !== 1,
      callback: (result) => {
  ...
  return (
    <div style={{ display: 'flex' }}>
      ...
      <Tooltip title={!isMobile && 'Xem trước in'}>
        <Button icon={<EyeOutlined />} onClick={onPrint} />
      </Tooltip>
    </div>
  );
};

export default PrintFormSelect;
