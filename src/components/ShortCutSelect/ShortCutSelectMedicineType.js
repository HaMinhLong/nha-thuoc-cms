...
const ShortCutSelectSupplierGroup = ({
  ...
  const [visibleAddMedicineType, setVisibleAddMedicineType] = useState(false);
  ...
  const handleSubmit = () => {
    setLoading(true);
    const addItem = {
      medicineTypeName: (medicineTypeName && medicineTypeName.trim()) || '',
      status: 1,
    };
    dispatch({
      ...
      callback: (res) => {
        ...
        if (res?.success) {
          ...
          setVisibleAddMedicineType(false);
          fetch(1, undefined, valueState, false, false, false);
        } else {
    ...
    <React.Fragment>
      <div style={{ display: 'inline-flex', width: '100%' }}>
        ...
        <Tooltip
          title={
            !isMobile &&
            intl.formatMessage({ id: 'app.medicineType.quickCreate.header' })
          }
        >
          <Button
            style={{
            ...
            type="ghost"
            icon={<PlusOutlined />}
            onClick={() => setVisibleAddMedicineType(!visibleAddMedicineType)}
          />
        </Tooltip>
      </div>
      <Spin spinning={loading}>
        <Modal
          ...
          title={intl.formatMessage({
            id: 'app.medicineType.quickCreate.header',
          })}
          visible={visibleAddMedicineType}
          onOk={handleSubmit}
          width={isMobile ? '100%' : '520px'}
          onCancel={() => setVisibleAddMedicineType(false)}
          cancelText={
            <React.Fragment>
          ...
          <FormItem
            ...
            label={
              <span>
                ...
                {intl.formatMessage({ id: 'app.medicineType.list.col0' })}
              </span>
            }
            rules={[
              {
