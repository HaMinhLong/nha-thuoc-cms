...
const ShortCutSelectSupplierGroup = ({
  ...
  const [visibleAddCustomerGroup, setVisibleAddCustomerGroup] = useState(false);
  ...
  const handleSubmit = () => {
    setLoading(true);
    const addItem = {
      customerGroupName: (customerGroupName && customerGroupName.trim()) || '',
      status: 1,
    ...
    dispatch({
      ...
      callback: (res) => {
        ...
        if (res?.success) {
          ...
          setVisibleAddCustomerGroup(false);
          fetch(1, undefined, valueState, false, false, false);
        } else {
    ...
    <React.Fragment>
      <div style={{ display: 'inline-flex', width: '100%' }}>
        ...
        <Tooltip
          title={
            !isMobile &&
            intl.formatMessage({ id: 'app.customerGroup.quickCreate.header' })
          }
        >
          <Button
            style={{
            ...
            type="ghost"
            icon={<PlusOutlined />}
            onClick={() => setVisibleAddCustomerGroup(!visibleAddCustomerGroup)}
          />
        </Tooltip>
      </div>
      <Spin spinning={loading}>
        <Modal
          ...
          title={intl.formatMessage({
            id: 'app.customerGroup.quickCreate.header',
          })}
          visible={visibleAddCustomerGroup}
          onOk={handleSubmit}
          width={isMobile ? '100%' : '520px'}
          onCancel={() => setVisibleAddCustomerGroup(false)}
          cancelText={
            <React.Fragment>
